use dwt_watermark_core::algorithm::*;
...
pub fn extract_watermark_from_png_bytes(png_bytes: Buffer) -> Result<String> {
    ...
    let extracted = dwt_extract_from_rgba(rgba.as_raw(), width, height)
...
pub fn embed_watermark_to_png_bytes(
    ...
    let width = rgba.width() as usize;
    let height = rgba.height() as usize;
    let embedded = dwt_embed_to_rgba(rgba.as_raw(), width, height, &watermark_text)
